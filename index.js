...
async function posterInfo(event) {
  ...
  let infoKino = []; // куда будем записывать
  for (let i = 0; i < search.length; i++) {
    infoKino.push(arr[search[i]]);
  }
  ...
  let searchSelector = document.querySelector.bind(document);
  console.log(searchSelector('.container-poster-close'));
  ...
  content.querySelectorAll('li>p:last-child').forEach((item, index) => {
    item.textContent = infoKino[index];
  });
